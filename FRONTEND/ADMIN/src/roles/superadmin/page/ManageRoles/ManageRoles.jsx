...
const ManageRoles = ({ userInfo, handleLogout }) => {
    ...
    const openGrantAccessModal = async (role) => {
        ...
        try {
            ...
            const permRes = await axiosInstance.get(`${api_url}/api/admin/by-role?ids=${role.role_id}`);

            // 2. Fetch all modules
            const modulesRes = await axiosInstance.get(`${api_url}/api/admin/modules`);
    ...
    const handleSavePermissions = async () => {
        try {
            ...
            const response = await axiosInstance.post(`${api_url}/api/admin/assign`, payload);
