...
const useManageRequests = (userInfo) => {
    ...
    const fetchTechnicians = useCallback(async () => {
        const res = await axiosInstance.post(`${api_url}/api/admin/FetchTechniciansByDistrict`);
        return res.data?.data || [];
    }, []);

    const fetchRequests = useCallback(async () => {
        const payload = isSeller ? { district: sellerDistrict } : {};
        const res = await axiosInstance.post(`${api_url}/api/admin/FetchManualRequests`, payload);
        return res.data?.data || [];
    }, [isSeller, sellerDistrict]);

    const fetchDevices = useCallback(async () => {
        const payload = isSeller ? { district: sellerDistrict } : {};
        const res = await axiosInstance.post(`${api_url}/api/admin/FetchInstalledDevicesForRequests`, payload);
        return res.data?.data || [];
    }, [isSeller, sellerDistrict]);
    ...
    const assignManualRequest = async ({ task_id, technician_id }) => {
        try {
            await axiosInstance.post(`${api_url}/api/admin/AssignManualRequest`, { task_id, technician_id });
    ...
    const reassignManualRequest = async ({ task_id, technician_id }) => {
        try {
            await axiosInstance.post(`${api_url}/api/admin/ReAssignManualRequest`, { task_id, technician_id });
    ...
    const createManualRequest = async ({
        ...
        try {
            await axiosInstance.post(`${api_url}/api/admin/CreateManualRequest`, {
                wp_device_id,
                customer_user_id,
                assigned_technician_id,
